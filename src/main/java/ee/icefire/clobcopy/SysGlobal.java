...
public class SysGlobal {
  ...
  private static Logger LOG = Logger.getLogger(SysGlobal.class);
...
//--------------------------------------------------------------//
  ...
  private static boolean checkProperties() {
    ...
    if (!isPropertySet(LOCAL_PATH, cfg)){
      LOG.error(LOCAL_PATH + " parameter is missing");
      return false;
    }

    if (!isPropertySet(CVS_TAG, cfg) && !isPropertySet(FILE_PATH, cfg)){
      LOG.error(CVS_TAG + " or " + FILE_PATH + " parameters are missing");
      return false;
    }

    if (!isPropertySet(MODE, cfg)){
      LOG.error(MODE + " parameter is missing");
      return false;
    }

    if (!isPropertySet(DB_URL, cfg)){
      LOG.error(DB_URL + " parameter is missing");
      return false;
    }

    if (!isPropertySet(DB_USER, cfg)){
      LOG.error(DB_USER + " parameter is missing");
      return false;
    }

    if (!isPropertySet(DB_PASS, cfg)){
      LOG.error(DB_PASS + " parameter is missing");
      return false;
    }
